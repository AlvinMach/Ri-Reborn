...
const Products = ({ item }) => {
  return (
    <div className="relative text-center w-full sm:w-[90%] md:max-w-[500px] lg:max-w-[800px] mx-auto bg-white rounded-lg shadow-lg overflow-hidden transition-transform duration-300 transform hover:scale-105 my-6">
      ...
      <Link to="" className="text-blue-600">
        <div className="relative overflow-hidden">
          <img
            src={item.image}
            alt={item.name}
            className="w-full h-[300px] object-cover aspect-[20/9] transition-transform duration-300"
          />
      ...
      <h3 className="mt-4 text-lg text-gray-800 font-semibold px-4">{item.title}</h3>
